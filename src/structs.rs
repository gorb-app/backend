...
impl Guild {
    ...
    pub async fn fetch_amount(
        ...
        let rows = sqlx::query_as::<_, (String, String, String, Option<String>, Option<String>)>(
            "SELECT CAST(uuid AS VARCHAR), CAST(owner_uuid AS VARCHAR), name, description, icon 
            FROM guilds 
            ORDER BY name 
            LIMIT $1 OFFSET $2",
        )
        ...
        let guild_futures = rows.into_iter().map(|(guild_uuid_raw, owner_uuid_raw, name, description, icon)| async move {
